...
class WorkspaceService {
  ...
  Future<void> uploadDocument({
    ...
    if (visibility != 'private' && deskId == null) throw ArgumentError('A Shared Desk is required for shared documents.');
    if (visibility == 'custom' && recipientIds.where((id) => id != _uid).isEmpty) {
      throw ArgumentError('Select at least one other person for custom access.');
    }
    ...
    try {
      ...
      if (visibility == 'custom') {
        final rows = recipientIds
            .where((id) => id != _uid)
            .toSet()
            .map((id) => {'document_id': documentId, 'user_id': id, 'granted_by': _uid})
            .toList();
  ...
  Future<List<Map<String, dynamic>>> bills() async {
    final data = List<Map<String, dynamic>>.from(
      await _db.from('bills').select().order('created_at', ascending: false),
    );
  ...
  Future<List<Map<String, dynamic>>> tasks() async {
    final data = List<Map<String, dynamic>>.from(
      await _db.from('tasks').select().order('created_at', ascending: false),
    );
  ...
  Future<void> _attachMemberNames(
    ...
    for (final deskId in deskIds) {
      try {
        ...
        // The row can still render without a display name.
      }
    }
  }
}
