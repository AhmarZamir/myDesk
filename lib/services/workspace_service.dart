...
class WorkspaceService {
  ...
  Future<void> uploadDocument({
    ...
    if (visibility == 'custom' && recipientIds.where((id) => id != _uid).isEmpty) {
      throw ArgumentError('Select at least one other person.');
    }
    ...
    try {
      ...
      if (visibility == 'custom') {
        final rows = recipientIds.where((id) => id != _uid).toSet().map((id) => {
  ...
  Future<List<Map<String, dynamic>>> khataEntries() async {
    final data = List<Map<String, dynamic>>.from(await _db.from('khata_entries').select().order('created_at', ascending: false));
    final buddiesData = await buddies();
    final names = {for (final b in buddiesData) '${b['user_id']}': '${b['full_name']}'};
    for (final row in data) {
      final buddyId = row['buddy_user_id']?.toString();
      if (buddyId != null && names[buddyId] != null) row['buddy_name'] = names[buddyId];
    }
    return data;
  }
  ...
  Future<void> addKhataEntry({
    ...
    await _db.from('khata_entries').insert({
      'desk_id': deskId,
      'created_by': _uid,
      'creditor_id': theyOweMe ? _uid : (buddyUserId ?? _uid),
      'debtor_id': theyOweMe ? (buddyUserId ?? _uid) : _uid,
  ...
  Future<Map<String, dynamic>> dashboardSnapshot() async {
    ...
    final overdueBills = unpaidBills.where((b) {
      final due = DateTime.tryParse('${b['due_date'] ?? ''}');
      return due != null && due.isBefore(today);
    }).toList();
    final pendingTasks = tasksData.where((t) => t['status'] != 'completed').toList();
    final dueSoonTasks = pendingTasks.where((t) {
      final due = DateTime.tryParse('${t['due_date'] ?? ''}');
      return due != null && !due.isBefore(today) && !due.isAfter(soon);
    }).toList();
    final expiringDocs = docs.where((d) {
      final expiry = DateTime.tryParse('${d['expires_at'] ?? ''}');
      return expiry != null && !expiry.isBefore(today) && !expiry.isAfter(today.add(const Duration(days: 30)));
    }).toList();
    return {
      ...
      'open_khata': khata.where((e) => e['status'] == 'open').length,
  ...
  Future<void> _attachMemberNames(List<Map<String, dynamic>> rows, {required String idField, required String outputField}) async {
    ...
    for (final row in rows) {
      ...
      if (id == _uid) row[outputField] = ownName?.isNotEmpty == true ? ownName : 'You';
    }
    ...
    for (final deskId in deskIds) {
      try {
        ...
        for (final row in rows.where((r) => r['desk_id']?.toString() == deskId)) {
          final id = row[idField]?.toString();
          if (id != null && names[id] != null) row[outputField] = names[id];
        }
      } catch (_) {}
    }
  }
