...
class AuthService {
  ...
  Future<AuthResponse> signUp({required String fullName, required String email, required String password}) async {
    final response = await _client.auth.signUp(email: email.trim(), password: password, data: {'full_name': fullName.trim()});
    if (_client.auth.currentUser != null) await ensureProfile(fullName: fullName);
    return response;
  }
  ...
  Future<void> ensureProfile({String? fullName}) async {
    ...
    try {
      ...
      if (fullName?.trim().isNotEmpty == true) await updateProfile(fullName!.trim());
      return;
    } catch (_) {}
    ...
    await _client.from('profiles').insert({
      'id': user.id,
      'full_name': (fullName?.trim().isNotEmpty == true) ? fullName!.trim() : (metadataName?.isNotEmpty == true ? metadataName : fallbackName),
    });
  }
  ...
  Future<void> updateProfile(String fullName) async {
    ...
    await _client.from('profiles').update({'full_name': name, 'updated_at': DateTime.now().toIso8601String()}).eq('id', user.id);
  ...
  Future<String> uploadAvatar({required Uint8List bytes, required String fileName}) async {
    final user = currentUser;
    if (user == null) throw StateError('Not signed in');
    if (bytes.isEmpty) throw ArgumentError('Selected image is empty');
    if (bytes.length > 5 * 1024 * 1024) throw ArgumentError('Profile image must be 5 MB or smaller');

    final ext = fileName.toLowerCase().split('.').last;
    if (!const {'jpg', 'jpeg', 'png', 'webp'}.contains(ext)) throw ArgumentError('Use JPG, PNG, or WebP');
    final safeExt = ext == 'jpeg' ? 'jpg' : ext;
    final path = '${user.id}/profile.$safeExt';
    ...
    await _client.from('profiles').update({'avatar_url': cacheBusted, 'updated_at': DateTime.now().toIso8601String()}).eq('id', user.id);
    await _client.auth.updateUser(UserAttributes(data: {'avatar_url': cacheBusted}));
    return cacheBusted;
  }
